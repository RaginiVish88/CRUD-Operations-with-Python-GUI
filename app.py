...
db = client["jewellery_db"]
collection = db["items"]

#  Main Window
root = tk.Tk()
root.title("💎 Jewellery Store")
...
# Input Fields
tk.Label(root, text="Jewellery Name:", font=("Helvetica", 12, "bold"), bg="#fefae0").pack(pady=5)
    ...
    if name and price:
        try:
            ...
            if result.matched_count > 0:
                ...
                messagebox.showwarning("⚠ Not Found", f"No jewellery found with name '{name}'")
        except ValueError:
            messagebox.showerror("❌ Error", "Price must be a number!")
    else:
